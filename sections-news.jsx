...
function NewsSidebar({ current, exclude }) {
  ...
  return <SideShell catTitle="文章分類" cats={NEWS_CATS} catUrl={newsCatUrl} catCount={newsCount}
    current={current} recentTitle="最新文章"
    recent={recent.map(p => ({ href: newsUrl(p), meta: p.date, title: p.title }))} />;
}
...
function NewsArchive({ cat = "全部" }) {
  ...
  return (
    <ArchiveShell eyebrow="News · 最新消息" title="二象的" accent="觀察筆記"
      lead="設計、行銷與專案現場的觀察紀錄。"
      barText={(cat === "全部" ? "全部文章" : cat) + " · " + filtered.length + " 篇"}
      isEmpty={!filtered.length} emptyText="此分類目前還沒有文章。"
      sidebar={<NewsSidebar current={cat} />}>
...
function NewsArticle({ post }) {
  ...
  return (
    <ArticleShell heroColor={post.heroColor} heroImg={post.heroImg} heroAlt={post.title}
      heroLabel="文章主視覺" cat={post.cat} tags={post.tags} date={post.date} title={post.title}
      sidebar={<NewsSidebar current={post.cat} exclude={post.slug} />}
      back={{ href: newsCatUrl(post.cat), label: "← 回「" + post.cat + "」文章列表" }}
      next={next ? { href: newsUrl(next), label: "NEXT POST", title: next.title } : null}>
      {post.blocks.map((b, i) =>
      b.t === "h" ? <h2 className="news-h" key={i}>{b.v}</h2> :
      b.t === "p" ? <p className="lb-sec-text" key={i}>{b.v}</p> :
      b.t === "quote" ? <p className="lb-sec-text" key={i}>{b.v}</p> :
