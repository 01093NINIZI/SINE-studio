...
function WorkSidebar({ current, exclude }) {
  ...
  return <SideShell catTitle="專案分類" cats={WORK_CATS} catUrl={workCatUrl} catCount={workCount}
    current={current} recentTitle="其他專案"
    recent={recent.map(w => ({ href: workUrl(w), meta: w.cat, title: w.zh }))} />;
}
...
function WorkArchive({ cat = "全部" }) {
  ...
  return (
    <ArchiveShell eyebrow="Work · 執行專案" title="品牌的" accent="無限可能"
      lead="從品牌識別到廣告投放的專案紀錄。"
      barText={(cat === "全部" ? "全部專案" : cat) + " · " + filtered.length + " 件"}
      isEmpty={!filtered.length} emptyText="此分類目前還沒有專案。"
      sidebar={<WorkSidebar current={cat} />}>
...
function WorkArticle({ work }) {
  ...
  return (
    <ArticleShell heroColor={work.heroColor} heroImg={work.heroImg} heroAlt={work.zh}
      heroLabel={"主視覺 · " + work.en.toUpperCase()} cat={work.cat} tags={work.tags} title={work.zh}
      sidebar={<WorkSidebar current={work.cat} exclude={work.slug} />}
      back={{ href: workCatUrl(work.cat), label: "← 回「" + work.cat + "」專案列表" }}
      next={next ? { href: workUrl(next), label: "NEXT PROJECT", title: next.zh } : null}>
...
function WorkCard({ work: w, delay = 0 }) {
  return (
    <a className="work-card reveal" href={workUrl(w)}
      ...
      <div className="meta">
        <h3>{w.zh}</h3>
        {(w.excerpt || w.about) && <p className="news-excerpt">{w.excerpt || w.about}</p>}
        <div className="tags">
    ...
    <section id="work" className="section" style={{ position: "relative", overflow: "hidden" }}>
      ...
      <div className="wrap">
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", flexWrap: "wrap", gap: 24 }}>
          <div>
            <a href="work.html" className="eyebrow reveal" style={{ textDecoration: "none" }}>Work · 執行專案</a>
...
/* ---- Team ---- */
const TEAM = [
{ zh: "Yu-Ting", role: "Founder · 創辦人", badge: "創辦人", img: "assets/team-founder.jpg", d: "堅持從 0 到 1，用專業與熱情完成真正喜歡的案子，注重品質與細節。",
  skills: ["專案管理", "企劃（電商、集資、社群、廣告）", "文案撰寫（銷售頁、社群、廣告）", "KOL 媒合行銷"] },
{ zh: "尼尼子", role: "Designer · 視覺設計", badge: "設計師", img: "assets/team-ninizi.jpg", d: "從品牌邏輯到視覺落點，職人式細節控，為品牌與市場打造有策略的視覺連接。",
  skills: ["品牌識別系統設計（LOGO、VIS…）", "廣告/電商設計（banner、銷售頁）", "包裝設計", "平面視覺（海報、印刷刊物、插畫）"] },
{ zh: "YT", role: "Marketing · 行銷企劃", badge: "行銷", img: "assets/team-yang.jpg", d: "整合社群、廣告與數據，讓設計的價值真正轉化為市場成效。",
  ...
  return (
    <section id="contact" className="section cta">
      ...
      <div className="wrap cta-inner">
        <div>
          <a href="contact.html" className="eyebrow on-dark reveal" style={{ textDecoration: "none" }}>Contact · 聯繫我們</a>
        ...
        <div className="foot-top">
          ...
          <div className="foot-cols">
            <div className="foot-col">
              <h4>Sitemap</h4>
              {NAV.map((n) => <a key={n.id} href={n.href}>{n.zh}</a>)}
            </div>
            <div className="foot-col">
              <h4>Services</h4>
              <a href={workCatUrl("電商代操")}>電商代操</a>
              <a href={workCatUrl("廣告投放")}>廣告投放</a>
              <a href={workCatUrl("平面設計")}>平面設計</a>
              <a href={workCatUrl("群眾集資")}>群眾集資</a>
            </div>
            <div className="foot-col">
              <h4>Contact</h4>
